...
<h1 align="center">🦀 Hrafn — Personal AI Assistant</h1>
...
> **Preferred setup:** run `hrafn onboard` in your terminal. Hrafn Onboard guides you step by step through setting up the gateway, workspace, channels, and provider. It is the recommended setup path and works on macOS, Linux, and Windows (via WSL2). New install? Start here: [Getting started](#quick-start)
...
`hrafn onboard` runs automatically after install to configure your workspace and provider.
...
Upgrading? Run `hrafn doctor` after updating.
...
Hrafn can import your OpenClaw workspace, memory, and configuration:
...
This migrates your memory entries, workspace files, and configuration from `~/.openclaw/` to `~/.hrafn/`. Config is converted from JSON to TOML automatically.

## Security defaults (DM access)

Hrafn connects to real messaging surfaces. Treat inbound DMs as untrusted input.
...
- Approve with: `hrafn pairing approve <channel> <code>` (then the sender is added to a local allowlist).
- Public inbound DMs require an explicit opt-in in `config.toml`.
- Run `hrafn doctor` to surface risky or misconfigured DM policies.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critical_  | We are **not affiliated** with `openagen/hrafn`, `hrafn.org` or `hrafn.net`. The `hrafn.org` and `hrafn.net` domains currently points to the `openagen/hrafn` fork, and that domain/repository are impersonating our official website/project.                                                                                       | Do not trust information, binaries, fundraising, or announcements from those sources. Use only [this repository](https://github.com/5queezer/hrafn) and our verified social accounts.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Important_ | Our official website is now live: [hrafn.ai](https://hrafn.ai). Thanks for your patience while we prepared the launch. We are still seeing impersonation attempts, so do **not** join any investment or fundraising activity claiming the Hrafn name unless it is published through our official channels.                            | Use [this repository](https://github.com/5queezer/hrafn) as the single source of truth. Follow [X (@hrafn)](https://x.com/hrafn?s=21), [Facebook (Group)](https://www.facebook.com/groups/hrafn), and [Reddit (r/hrafn)](https://www.reddit.com/r/hrafn/) for official updates. |
...
> Notes: Hrafn results are measured on release builds using `/usr/bin/time -l`. OpenClaw requires Node.js runtime (typically ~390MB additional memory overhead), while NanoBot requires Python runtime. PicoClaw and Hrafn are static binaries. The RAM figures above are runtime memory; build-time compilation requirements are higher.
...
Hrafn supports subscription-native auth profiles (multi-account, encrypted at rest).
...
Workspace root: `~/.hrafn/workspace/` (configurable via config).
...
Skills: `~/.hrafn/workspace/skills/<skill>/SKILL.md` or `SKILL.toml`.
...
Or skip the steps above and install everything (system deps, Rust, Hrafn) in a single command:
...
- Follow [Hrafn Onboard](#quick-start) for a guided setup.
...
Hrafn was built for the smooth crab 🦀, a fast and efficient AI assistant. Built by Argenis De La Rosa and the community.
...
If Hrafn helps your work and you want to support ongoing development, you can donate here:
...
New to Hrafn? Look for issues labeled [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — see our [Contributing Guide](CONTRIBUTING.md#first-time-contributors) for how to get started. AI/vibe-coded PRs welcome! 🤖
...
- New `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**This is the only official Hrafn repository:**

> https://github.com/5queezer/hrafn

Any other repository, organization, domain, or package claiming to be "Hrafn" or implying affiliation with Hrafn Labs is **unauthorized and not affiliated with this project**. Known unauthorized forks will be listed in [TRADEMARK.md](docs/maintainers/trademark.md).

If you encounter impersonation or trademark misuse, please [open an issue](https://github.com/5queezer/hrafn/issues).

---

## License

Hrafn is dual-licensed for maximum openness and contributor protection:
...
The **Hrafn** name and logo are trademarks of Hrafn Labs. This license does not grant permission to use them to imply endorsement or affiliation. See [TRADEMARK.md](docs/maintainers/trademark.md) for permitted and prohibited uses.
...
**Hrafn** — Zero overhead. Zero compromise. Deploy anywhere. Swap anything. 🦀
