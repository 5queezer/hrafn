...
<h1 align="center">🦀 Hrafn — Assistant Personnel IA</h1>
...
`hrafn onboard` s'exécute automatiquement après l'installation pour configurer votre workspace et fournisseur.
...
hrafn gateway                # par défaut : 127.0.0.1:42617
hrafn gateway --port 0       # port aléatoire (sécurité renforcée)
...
Mise à jour ? Exécutez `hrafn doctor` après la mise à jour.
...
Hrafn peut importer votre workspace, mémoire et configuration OpenClaw :
...
Cela migre vos entrées de mémoire, fichiers du workspace et configuration de `~/.openclaw/` vers `~/.hrafn/`. La configuration est convertie de JSON en TOML automatiquement.

## Paramètres de sécurité par défaut (accès DM)

Hrafn se connecte à de vraies surfaces de messagerie. Traitez les DM entrants comme des entrées non fiables.
...
- Approuver avec : `hrafn pairing approve <channel> <code>` (l'expéditeur est alors ajouté à une liste d'autorisation locale).
- Les DM publics entrants nécessitent une activation explicite dans `config.toml`.
- Exécutez `hrafn doctor` pour détecter les politiques DM risquées ou mal configurées.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/hrafn`, `hrafn.org` ou `hrafn.net`. Les domaines `hrafn.org` et `hrafn.net` pointent actuellement vers le fork `openagen/hrafn`, et ce domaine/dépôt usurpent l'identité de notre site web/projet officiel.                                                                                       | Ne faites pas confiance aux informations, binaires, collectes de fonds ou annonces provenant de ces sources. Utilisez uniquement [ce dépôt](https://github.com/5queezer/hrafn) et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Pourquoi les équipes choisissent Hrafn
...
## Résumé des benchmarks (Hrafn vs OpenClaw, reproductible)
...
> Notes : Les résultats de Hrafn sont mesurés sur des builds release avec `/usr/bin/time -l`. OpenClaw nécessite le runtime Node.js (typiquement ~390Mo de surcharge mémoire supplémentaire), tandis que NanoBot nécessite le runtime Python. PicoClaw et Hrafn sont des binaires statiques. Les chiffres de RAM ci-dessus sont la mémoire à l'exécution ; les besoins de compilation sont plus élevés.
...
Hrafn supporte les profils d'authentification natifs par abonnement (multi-compte, chiffrés au repos).

- Fichier de stockage : `~/.hrafn/auth-profiles.json`
- Clé de chiffrement : `~/.hrafn/.secret_key`
...
Racine du workspace : `~/.hrafn/workspace/` (configurable via config).
...
hrafn onboard              # Assistant de configuration guidée
hrafn status               # Afficher le statut du daemon/agent
hrafn doctor               # Exécuter les diagnostics système

# Gateway + daemon
hrafn gateway              # Démarrer le serveur gateway (127.0.0.1:42617)
hrafn daemon               # Démarrer le runtime autonome complet

# Agent
hrafn agent                # Mode chat interactif
hrafn agent -m "message"   # Mode message unique

# Gestion des services
hrafn service install      # Installer comme service OS (launchd/systemd)
hrafn service start|stop|restart|status

# Canaux
hrafn channel list         # Lister les canaux configurés
hrafn channel doctor       # Vérifier la santé des canaux
hrafn channel bind-telegram 123456789

# Cron + planification
hrafn cron list            # Lister les tâches planifiées
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Mémoire
hrafn memory list          # Lister les entrées de mémoire
hrafn memory get <key>     # Récupérer une mémoire
hrafn memory stats         # Statistiques de la mémoire
...
hrafn hardware discover    # Scanner les appareils connectés
hrafn peripheral list      # Lister les périphériques connectés
hrafn peripheral flash     # Flasher le firmware sur l'appareil
...
Ou passez les étapes ci-dessus et installez tout (dépendances système, Rust, Hrafn) en une seule commande :
...
- Suivez [Hrafn Onboard](#démarrage-rapide) pour une configuration guidée.
...
Hrafn a été construit pour le crabe lisse 🦀, un assistant IA rapide et efficace. Construit par Argenis De La Rosa et la communauté.
...
Si Hrafn vous aide dans votre travail et que vous souhaitez soutenir le développement continu, vous pouvez faire un don ici :
...
Nouveau sur Hrafn ? Recherchez les issues étiquetées [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — consultez notre [Guide de contribution](CONTRIBUTING.md#first-time-contributors) pour savoir comment commencer. Les PRs IA/vibe-coded sont les bienvenus ! 🤖
...
- Nouveau `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**Ceci est le seul dépôt officiel de Hrafn :**

> https://github.com/5queezer/hrafn

Tout autre dépôt, organisation, domaine ou package prétendant être "Hrafn" ou impliquant une affiliation avec Hrafn Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](docs/maintainers/trademark.md).

Si vous rencontrez une usurpation d'identité ou un usage abusif de la marque, veuillez [ouvrir une issue](https://github.com/5queezer/hrafn/issues).

---

## Licence

Hrafn est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom et le logo **Hrafn** sont des marques de Hrafn Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer un soutien ou une affiliation. Voir [TRADEMARK.md](docs/maintainers/trademark.md) pour les usages autorisés et interdits.
...
**Hrafn** — Zéro overhead. Zéro compromis. Déployez partout. Échangez n'importe quoi. 🦀
