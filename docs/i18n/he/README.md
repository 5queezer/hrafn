...
<h1 align="center">🦀 Hrafn — עוזר AI אישי</h1>
...
> **הגדרה מועדפת:** הרץ `hrafn onboard` בטרמינל שלך. Hrafn Onboard מנחה אותך שלב אחר שלב בהגדרת ה-gateway, סביבת העבודה, הערוצים והספק. זהו נתיב ההגדרה המומלץ ועובד על macOS, Linux ו-Windows (דרך WSL2). התקנה חדשה? התחל כאן: [התחלה](#התחלה-מהירה)
...
`hrafn onboard` רץ אוטומטית לאחר ההתקנה כדי להגדיר את סביבת העבודה והספק שלך.
...
משדרג? הרץ `hrafn doctor` לאחר העדכון.
...
Hrafn יכול לייבא את סביבת העבודה, הזיכרון וההגדרות של OpenClaw שלך:
...
זה מעביר את רשומות הזיכרון, קבצי סביבת העבודה וההגדרות מ-`~/.openclaw/` ל-`~/.hrafn/`. ההגדרות מומרות אוטומטית מ-JSON ל-TOML.

## ברירות מחדל אבטחה (גישת DM)

Hrafn מתחבר למשטחי הודעות אמיתיים. התייחס ל-DM נכנסים כקלט לא מהימן.
...
- אשר עם: `hrafn pairing approve <channel> <code>` (ואז השולח נוסף לרשימת היתרים מקומית).
- DM נכנסים ציבוריים דורשים הסכמה מפורשת ב-`config.toml`.
- הרץ `hrafn doctor` כדי לחשוף מדיניות DM מסוכנת או שגויה.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _קריטי_ | אנחנו **לא מזוהים** עם `openagen/hrafn`, `hrafn.org` או `hrafn.net`. הדומיינים `hrafn.org` ו-`hrafn.net` מפנים כרגע ל-fork `openagen/hrafn`, ואותו דומיין/מאגר מתחזים לאתר/פרויקט הרשמי שלנו. | אל תסמוך על מידע, בינאריים, גיוס כספים או הודעות ממקורות אלה. השתמש רק ב[מאגר זה](https://github.com/5queezer/hrafn) ובחשבונות החברתיים המאומתים שלנו. |
...
## תמונת מצב של ביצועים (Hrafn מול OpenClaw, ניתן לשחזור)
...
> הערות: תוצאות Hrafn נמדדו על בנייות שחרור באמצעות `/usr/bin/time -l`. OpenClaw דורש סביבת ריצה Node.js (בדרך כלל ~390MB תקורת זיכרון נוספת), בעוד NanoBot דורש סביבת ריצה Python. PicoClaw ו-Hrafn הם בינאריים סטטיים. נתוני ה-RAM למעלה הם זיכרון סביבת ריצה; דרישות קומפילציה בזמן בנייה גבוהות יותר.
...
Hrafn תומך בפרופילי אימות מקוריים למנוי (רב-חשבוני, מוצפן במנוחה).

- קובץ אחסון: `~/.hrafn/auth-profiles.json`
- מפתח הצפנה: `~/.hrafn/.secret_key`
...
שורש סביבת עבודה: `~/.hrafn/workspace/` (ניתן להגדרה דרך ההגדרות).
...
מיומנויות: `~/.hrafn/workspace/skills/<skill>/SKILL.md` או `SKILL.toml`.
...
או דלג על השלבים למעלה והתקן הכל (תלויות מערכת, Rust, Hrafn) בפקודה אחת:
...
- עקוב אחרי [Hrafn Onboard](#התחלה-מהירה) להגדרה מונחית.
...
Hrafn נבנה עבור ה-smooth crab 🦀, עוזר AI מהיר ויעיל. נבנה על ידי Argenis De La Rosa והקהילה.
...
אם Hrafn עוזר לעבודה שלך ואתה רוצה לתמוך בפיתוח המתמשך, אתה יכול לתרום כאן:
...
חדש ב-Hrafn? חפש בעיות עם התווית [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ראה את [מדריך התרומה](CONTRIBUTING.md#first-time-contributors) שלנו כדי להתחיל. PR של AI/vibe-coded מתקבלים בברכה! 🤖
...
- `Skill` חדש → `~/.hrafn/workspace/skills/<name>/`
...
**זהו מאגר Hrafn הרשמי היחיד:**

> https://github.com/5queezer/hrafn

כל מאגר, ארגון, דומיין או חבילה אחרים הטוענים להיות "Hrafn" או מרמזים על שיוך ל-Hrafn Labs הם **לא מורשים ולא מזוהים עם פרויקט זה**. פורקים לא מורשים ידועים ירשמו ב-[TRADEMARK.md](docs/maintainers/trademark.md).

אם אתה נתקל בהתחזות או שימוש לרעה בסימן מסחרי, אנא [פתח issue](https://github.com/5queezer/hrafn/issues).

---

## רישיון

Hrafn מורשה ברישיון כפול לפתיחות מקסימלית והגנה על תורמים:
...
השם והלוגו של **Hrafn** הם סימנים מסחריים של Hrafn Labs. רישיון זה אינו מעניק הרשאה להשתמש בהם כדי לרמוז על תמיכה או שיוך. ראה [TRADEMARK.md](docs/maintainers/trademark.md) לשימושים מותרים ואסורים.
...
**Hrafn** — אפס תקורה. אפס פשרות. פרוס בכל מקום. החלף הכל. 🦀
