...
<h1 align="center">🦀 Hrafn — व्यक्तिगत AI सहायक</h1>
...
> **पसंदीदा सेटअप:** अपने टर्मिनल में `hrafn onboard` चलाएँ। Hrafn Onboard आपको gateway, workspace, channels, और provider सेट करने में कदम-दर-कदम मार्गदर्शन करता है। यह अनुशंसित सेटअप पथ है और macOS, Linux, और Windows (WSL2 के माध्यम से) पर काम करता है। नया इंस्टॉल? यहाँ से शुरू करें: [शुरू करें](#त्वरित-शुरुआत)
...
`hrafn onboard` इंस्टॉल के बाद स्वचालित रूप से चलता है ताकि आपका workspace और provider कॉन्फ़िगर हो सके।
...
अपग्रेड कर रहे हैं? अपडेट के बाद `hrafn doctor` चलाएँ।
...
Hrafn आपके OpenClaw workspace, मेमोरी, और कॉन्फ़िगरेशन आयात कर सकता है:
...
यह आपकी मेमोरी प्रविष्टियों, workspace फ़ाइलों, और कॉन्फ़िगरेशन को `~/.openclaw/` से `~/.hrafn/` में माइग्रेट करता है। कॉन्फ़िग स्वचालित रूप से JSON से TOML में परिवर्तित हो जाता है।

## सुरक्षा डिफ़ॉल्ट (DM एक्सेस)

Hrafn वास्तविक मैसेजिंग सतहों से जुड़ता है। इनबाउंड DMs को अविश्वसनीय इनपुट के रूप में मानें।
...
- इससे स्वीकृति दें: `hrafn pairing approve <channel> <code>` (फिर प्रेषक स्थानीय अनुमति सूची में जोड़ा जाता है)।
- सार्वजनिक इनबाउंड DMs के लिए `config.toml` में स्पष्ट ऑप्ट-इन आवश्यक है।
- जोखिमपूर्ण या गलत कॉन्फ़िगर DM नीतियों को सामने लाने के लिए `hrafn doctor` चलाएँ।
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _गंभीर_ | हम `openagen/hrafn`, `hrafn.org` या `hrafn.net` से **संबद्ध नहीं** हैं। `hrafn.org` और `hrafn.net` डोमेन वर्तमान में `openagen/hrafn` फ़ोर्क की ओर इशारा करते हैं, और वह डोमेन/रिपॉजिटरी हमारी आधिकारिक वेबसाइट/प्रोजेक्ट का रूप धारण कर रहे हैं। | उन स्रोतों से जानकारी, बाइनरी, फंडरेजिंग, या घोषणाओं पर भरोसा न करें। केवल [यह रिपॉजिटरी](https://github.com/5queezer/hrafn) और हमारे सत्यापित सोशल अकाउंट्स का उपयोग करें। |
...
## बेंचमार्क स्नैपशॉट (Hrafn बनाम OpenClaw, प्रतिलिपि योग्य)
...
> नोट: Hrafn परिणाम `/usr/bin/time -l` का उपयोग करके रिलीज़ बिल्ड पर मापे गए हैं। OpenClaw को Node.js रनटाइम की आवश्यकता है (आमतौर पर ~390MB अतिरिक्त मेमोरी ओवरहेड), जबकि NanoBot को Python रनटाइम की आवश्यकता है। PicoClaw और Hrafn स्टैटिक बाइनरी हैं। ऊपर दिए गए RAM आँकड़े रनटाइम मेमोरी हैं; बिल्ड-टाइम कंपाइलेशन आवश्यकताएँ अधिक हैं।
...
Hrafn सब्सक्रिप्शन-नेटिव ऑथ प्रोफ़ाइल का समर्थन करता है (मल्टी-अकाउंट, रेस्ट पर एन्क्रिप्टेड)।

- स्टोर फ़ाइल: `~/.hrafn/auth-profiles.json`
- एन्क्रिप्शन कुंजी: `~/.hrafn/.secret_key`
...
Workspace रूट: `~/.hrafn/workspace/` (कॉन्फ़िग के माध्यम से कॉन्फ़िगर करने योग्य)।
...
स्किल: `~/.hrafn/workspace/skills/<skill>/SKILL.md` या `SKILL.toml`।
...
या ऊपर के चरणों को छोड़ें और एक ही कमांड में सब कुछ (सिस्टम deps, Rust, Hrafn) इंस्टॉल करें:
...
- मार्गदर्शित सेटअप के लिए [Hrafn Onboard](#त्वरित-शुरुआत) का पालन करें।
...
Hrafn को smooth crab 🦀 के लिए बनाया गया था, एक तेज़ और कुशल AI सहायक। Argenis De La Rosa और समुदाय द्वारा निर्मित।

- [hrafn.ai](https://hrafn.ai)
- [@hrafn](https://x.com/hrafn)

## Hrafn का समर्थन करें

यदि Hrafn आपके काम में मदद करता है और आप चल रहे विकास का समर्थन करना चाहते हैं, तो आप यहाँ दान कर सकते हैं:
...
Hrafn में नए हैं? [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) लेबल वाले मुद्दों की तलाश करें — शुरू करने का तरीका जानने के लिए हमारा [योगदान गाइड](CONTRIBUTING.md#first-time-contributors) देखें। AI/vibe-coded PRs का स्वागत है! 🤖
...
- नया `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**यह एकमात्र आधिकारिक Hrafn रिपॉजिटरी है:**

> https://github.com/5queezer/hrafn

कोई भी अन्य रिपॉजिटरी, संगठन, डोमेन, या पैकेज जो "Hrafn" होने का दावा करता है या Hrafn Labs से संबद्धता का संकेत देता है, **अनधिकृत है और इस प्रोजेक्ट से संबद्ध नहीं है**। ज्ञात अनधिकृत फ़ोर्क [TRADEMARK.md](docs/maintainers/trademark.md) में सूचीबद्ध किए जाएँगे।

यदि आप प्रतिरूपण या ट्रेडमार्क दुरुपयोग का सामना करते हैं, तो कृपया [एक इश्यू खोलें](https://github.com/5queezer/hrafn/issues)।

---

## लाइसेंस

Hrafn अधिकतम खुलेपन और योगदानकर्ता सुरक्षा के लिए दोहरे-लाइसेंस प्राप्त है:
...
**Hrafn** नाम और लोगो Hrafn Labs के ट्रेडमार्क हैं। यह लाइसेंस समर्थन या संबद्धता का संकेत देने के लिए इनका उपयोग करने की अनुमति नहीं देता। अनुमत और निषिद्ध उपयोग के लिए [TRADEMARK.md](docs/maintainers/trademark.md) देखें।
...
**Hrafn** — शून्य ओवरहेड। शून्य समझौता। कहीं भी डिप्लॉय करें। कुछ भी स्वैप करें। 🦀
