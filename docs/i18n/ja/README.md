...
<h1 align="center">🦀 Hrafn — パーソナルAIアシスタント</h1>
...
> **推奨セットアップ：** ターミナルで `hrafn onboard` を実行してください。Hrafn Onboardがゲートウェイ、ワークスペース、チャンネル、プロバイダーのセットアップをステップバイステップでガイドします。これは推奨されるセットアップパスで、macOS、Linux、Windows（WSL2経由）で動作します。新規インストール？ここから開始：[はじめに](#クイックスタートtldr)
...
`hrafn onboard` はインストール後に自動的に実行され、ワークスペースとプロバイダーを設定します。
...
hrafn gateway                # デフォルト：127.0.0.1:42617
hrafn gateway --port 0       # ランダムポート（セキュリティ強化）
...
アップグレード？更新後に `hrafn doctor` を実行してください。
...
HrafnはOpenClawのワークスペース、メモリ、設定をインポートできます：
...
これにより、メモリエントリ、ワークスペースファイル、設定が `~/.openclaw/` から `~/.hrafn/` に移行されます。設定はJSONからTOMLに自動変換されます。

## セキュリティデフォルト（DMアクセス）

Hrafnは実際のメッセージングサービスに接続します。着信DMを信頼できない入力として扱ってください。
...
- 承認方法：`hrafn pairing approve <channel> <code>`（送信者がローカル許可リストに追加されます）。
- パブリック着信DMには `config.toml` での明示的なオプトインが必要です。
- `hrafn doctor` を実行してリスクのある、または設定ミスのあるDMポリシーを検出します。
...
| 日付 (UTC) | レベル | 通知 | 対応 |
| ---------- | ------ | ---- | ---- |
| 2026-02-19 | _重大_ | 当プロジェクトは `openagen/hrafn`、`hrafn.org`、`hrafn.net` とは**一切関係ありません**。`hrafn.org` と `hrafn.net` ドメインは現在 `openagen/hrafn` フォークを指しており、そのドメイン/リポジトリは当プロジェクトの公式ウェブサイト/プロジェクトを偽装しています。 | それらのソースからの情報、バイナリ、資金調達、告知を信頼しないでください。[このリポジトリ](https://github.com/5queezer/hrafn)と認証済みのソーシャルアカウントのみを使用してください。 |
...
## ベンチマークスナップショット（Hrafn vs OpenClaw、再現可能）
...
> 注意：Hrafnの結果はリリースビルドで `/usr/bin/time -l` を使用して測定されています。OpenClawにはNode.jsランタイム（通常約390MBの追加メモリオーバーヘッド）が必要で、NanoBotにはPythonランタイムが必要です。PicoClawとHrafnは静的バイナリです。上記のRAM数値はランタイムメモリです。ビルド時のコンパイル要件はより高くなります。
...
Hrafnはサブスクリプションネイティブ認証プロファイル（マルチアカウント、保存時暗号化）をサポートしています。

- ストアファイル：`~/.hrafn/auth-profiles.json`
- 暗号化キー：`~/.hrafn/.secret_key`
...
ワークスペースルート：`~/.hrafn/workspace/`（設定で変更可能）。
...
スキル：`~/.hrafn/workspace/skills/<skill>/SKILL.md` または `SKILL.toml`。
...
```bash
# ワークスペース管理
hrafn onboard              # ガイド付きセットアップウィザード
hrafn status               # デーモン/エージェントのステータス表示
hrafn doctor               # システム診断を実行

# ゲートウェイ + デーモン
hrafn gateway              # ゲートウェイサーバーを起動（127.0.0.1:42617）
hrafn daemon               # フル自律ランタイムを起動

# エージェント
hrafn agent                # インタラクティブチャットモード
hrafn agent -m "message"   # 単一メッセージモード

# サービス管理
hrafn service install      # OSサービスとしてインストール（launchd/systemd）
hrafn service start|stop|restart|status

# チャンネル
hrafn channel list         # 設定済みチャンネルの一覧
hrafn channel doctor       # チャンネルヘルスの確認
hrafn channel bind-telegram 123456789

# Cron + スケジューリング
hrafn cron list            # スケジュールタスクの一覧
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# メモリ
hrafn memory list          # メモリエントリの一覧
hrafn memory get <key>     # メモリの取得
hrafn memory stats         # メモリ統計
...
# ハードウェア周辺機器
hrafn hardware discover    # 接続デバイスのスキャン
hrafn peripheral list      # 接続周辺機器の一覧
hrafn peripheral flash     # デバイスへのファームウェア書き込み
...
または、上記のステップをスキップして、単一コマンドですべてをインストール（システム依存、Rust、Hrafn）：
...
- [Hrafn Onboard](#クイックスタートtldr)でガイド付きセットアップ。
...
Hrafnはsmooth crab 🦀のために構築されました。高速で効率的なAIアシスタント。Argenis De La Rosaとコミュニティによって構築されました。
...
Hrafnがあなたの仕事に役立ち、継続的な開発を支援したい場合は、こちらから寄付できます：
...
Hrafn初心者ですか？[`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルの付いた課題を探してください — 始め方は[貢献ガイド](CONTRIBUTING.md#first-time-contributors)を参照。AI/vibe-coded PRも歓迎します！🤖
...
- 新 `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**これがHrafnの唯一の公式リポジトリです：**

> https://github.com/5queezer/hrafn

「Hrafn」を名乗る、またはHrafn Labsとの提携を示唆する他のリポジトリ、組織、ドメイン、パッケージは**無許可であり、本プロジェクトとは無関係です**。既知の無許可フォークは [TRADEMARK.md](docs/maintainers/trademark.md) に記載されます。

偽装や商標の悪用を見つけた場合は、[issueを作成](https://github.com/5queezer/hrafn/issues)してください。

---

## ライセンス

Hrafnは最大限のオープン性と貢献者保護のためにデュアルライセンスです：
...
**Hrafn** の名称とロゴはHrafn Labsの商標です。このライセンスは、推薦や提携を暗示するための使用許可を付与しません。許可された使用と禁止された使用については [TRADEMARK.md](docs/maintainers/trademark.md) を参照してください。
...
**Hrafn** — ゼロオーバーヘッド。ゼロ妥協。どこでもデプロイ。何でも交換。🦀
