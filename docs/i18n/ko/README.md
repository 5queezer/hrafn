...
<h1 align="center">🦀 Hrafn — 개인 AI 어시스턴트</h1>
...
> **권장 설정:** 터미널에서 `hrafn onboard`를 실행하세요. Hrafn Onboard가 gateway, workspace, 채널, 제공자 설정을 단계별로 안내합니다. macOS, Linux, Windows(WSL2)에서 작동하는 권장 설정 경로입니다. 새로 설치하시나요? 여기서 시작하세요: [시작하기](#빠른-시작-tldr)
...
`hrafn onboard`는 설치 후 자동으로 실행되어 workspace와 제공자를 구성합니다.
...
hrafn gateway                # 기본값: 127.0.0.1:42617
hrafn gateway --port 0       # 랜덤 포트 (보안 강화)
...
업그레이드 하셨나요? 업데이트 후 `hrafn doctor`를 실행하세요.
...
Hrafn는 OpenClaw workspace, 메모리, 구성을 가져올 수 있습니다:
...
이것은 메모리 항목, workspace 파일, 구성을 `~/.openclaw/`에서 `~/.hrafn/`로 마이그레이션합니다. 구성은 JSON에서 TOML로 자동 변환됩니다.

## 보안 기본값 (DM 접근)

Hrafn는 실제 메시징 서비스에 연결됩니다. 수신 DM을 신뢰할 수 없는 입력으로 취급하세요.
...
- 승인: `hrafn pairing approve <channel> <code>` (발신자가 로컬 허용 목록에 추가됩니다).
- 공개 수신 DM은 `config.toml`에서 명시적 옵트인이 필요합니다.
- `hrafn doctor`를 실행하여 위험하거나 잘못 구성된 DM 정책을 확인하세요.
...
| 날짜 (UTC) | 수준 | 공지 | 조치 |
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _치명적_ | 우리는 `openagen/hrafn`, `hrafn.org` 또는 `hrafn.net`과 **관련이 없습니다**. `hrafn.org`과 `hrafn.net` 도메인은 현재 `openagen/hrafn` 포크를 가리키고 있으며, 해당 도메인/저장소는 우리의 공식 웹사이트/프로젝트를 사칭하고 있습니다. | 해당 소스의 정보, 바이너리, 모금, 공지를 신뢰하지 마세요. [이 저장소](https://github.com/5queezer/hrafn)와 검증된 소셜 계정만 사용하세요. |
...
## 벤치마크 스냅샷 (Hrafn vs OpenClaw, 재현 가능)
...
> 참고: Hrafn 결과는 `/usr/bin/time -l`을 사용한 릴리스 빌드에서 측정되었습니다. OpenClaw는 Node.js 런타임이 필요하며(일반적으로 ~390MB 추가 메모리 오버헤드), NanoBot은 Python 런타임이 필요합니다. PicoClaw와 Hrafn는 정적 바이너리입니다. 위 RAM 수치는 런타임 메모리이며, 빌드 시 컴파일 요구사항은 더 높습니다.
...
Hrafn는 구독 기반 인증 프로필(다중 계정, 저장 시 암호화)을 지원합니다.

- 저장 파일: `~/.hrafn/auth-profiles.json`
- 암호화 키: `~/.hrafn/.secret_key`
...
Workspace 루트: `~/.hrafn/workspace/` (구성을 통해 변경 가능).
...
스킬: `~/.hrafn/workspace/skills/<skill>/SKILL.md` 또는 `SKILL.toml`.
...
```bash
# Workspace 관리
hrafn onboard              # 안내된 설정 마법사
hrafn status               # 데몬/에이전트 상태 표시
hrafn doctor               # 시스템 진단 실행

# Gateway + 데몬
hrafn gateway              # Gateway 서버 시작 (127.0.0.1:42617)
hrafn daemon               # 완전 자율 런타임 시작

# 에이전트
hrafn agent                # 대화형 채팅 모드
hrafn agent -m "message"   # 단일 메시지 모드

# 서비스 관리
hrafn service install      # OS 서비스로 설치 (launchd/systemd)
hrafn service start|stop|restart|status

# 채널
hrafn channel list         # 구성된 채널 목록
hrafn channel doctor       # 채널 상태 확인
hrafn channel bind-telegram 123456789

# Cron + 스케줄링
hrafn cron list            # 예약된 작업 목록
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# 메모리
hrafn memory list          # 메모리 항목 목록
hrafn memory get <key>     # 메모리 조회
hrafn memory stats         # 메모리 통계
...
# 하드웨어 주변기기
hrafn hardware discover    # 연결된 기기 스캔
hrafn peripheral list      # 연결된 주변기기 목록
hrafn peripheral flash     # 기기에 펌웨어 플래시
...
위 단계를 건너뛰고 모든 것(시스템 의존성, Rust, Hrafn)을 한 번에 설치:
...
- [Hrafn Onboard](#빠른-시작-tldr)를 따라 안내된 설정을 진행하세요.
...
Hrafn는 빠르고 효율적인 AI 어시스턴트인 smooth crab 🦀을 위해 만들어졌습니다. Argenis De La Rosa와 커뮤니티가 만들었습니다.
...
Hrafn가 여러분의 작업에 도움이 되었고 지속적인 개발을 지원하고 싶다면 여기에서 기부할 수 있습니다:
...
Hrafn가 처음이신가요? [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) 레이블이 붙은 이슈를 찾아보세요 — 시작하는 방법은 [기여 가이드](CONTRIBUTING.md#first-time-contributors)를 참조하세요. AI/vibe-coded PR도 환영합니다! 🤖
...
- 새 `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**이것이 유일한 공식 Hrafn 저장소입니다:**

> https://github.com/5queezer/hrafn

"Hrafn"라고 주장하거나 Hrafn Labs와의 제휴를 암시하는 다른 저장소, 조직, 도메인 또는 패키지는 **승인되지 않았으며 이 프로젝트와 관련이 없습니다**. 알려진 비인가 포크는 [TRADEMARK.md](docs/maintainers/trademark.md)에 나열됩니다.

사칭이나 상표 오용을 발견하면 [이슈를 열어](https://github.com/5queezer/hrafn/issues) 신고해 주세요.

---

## 라이선스

Hrafn는 최대한의 개방성과 기여자 보호를 위해 듀얼 라이선스가 적용됩니다:
...
**Hrafn** 이름과 로고는 Hrafn Labs의 상표입니다. 이 라이선스는 승인이나 제휴를 암시하기 위해 사용할 권한을 부여하지 않습니다. 허용 및 금지 사용은 [TRADEMARK.md](docs/maintainers/trademark.md)를 참조하세요.
...
**Hrafn** — 오버헤드 없음. 타협 없음. 어디서나 배포. 무엇이든 교체. 🦀
