...
<h1 align="center">🦀 Hrafn — Assistente Pessoal de IA</h1>
...
> **Configuração preferida:** execute `hrafn onboard` no seu terminal. O Hrafn Onboard guia você passo a passo na configuração do gateway, workspace, canais e provedor. É o caminho de configuração recomendado e funciona no macOS, Linux e Windows (via WSL2). Nova instalação? Comece aqui: [Primeiros passos](#início-rápido)
...
`hrafn onboard` executa automaticamente após a instalação para configurar seu workspace e provedor.
...
hrafn gateway                # padrão: 127.0.0.1:42617
hrafn gateway --port 0       # porta aleatória (segurança reforçada)
...
Atualizando? Execute `hrafn doctor` após atualizar.
...
O Hrafn pode importar seu workspace, memória e configuração do OpenClaw:
...
Isso migra suas entradas de memória, arquivos do workspace e configuração de `~/.openclaw/` para `~/.hrafn/`. A configuração é convertida de JSON para TOML automaticamente.

## Padrões de segurança (acesso por DM)

O Hrafn conecta-se a superfícies de mensagens reais. Trate DMs recebidas como entrada não confiável.
...
- Aprovar com: `hrafn pairing approve <channel> <code>` (então o remetente é adicionado a uma lista de permitidos local).
- DMs públicas recebidas requerem uma ativação explícita em `config.toml`.
- Execute `hrafn doctor` para detectar políticas de DM arriscadas ou mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **Não somos afiliados** a `openagen/hrafn`, `hrafn.org` ou `hrafn.net`. Os domínios `hrafn.org` e `hrafn.net` atualmente apontam para o fork `openagen/hrafn`, e esse domínio/repositório estão se passando pelo nosso site/projeto oficial.                                                                                       | Não confie em informações, binários, arrecadações de fundos ou anúncios dessas fontes. Use apenas [este repositório](https://github.com/5queezer/hrafn) e nossas contas sociais verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Por que equipes escolhem o Hrafn
...
## Resumo de benchmarks (Hrafn vs OpenClaw, reproduzível)
...
> Notas: Os resultados do Hrafn são medidos em builds release usando `/usr/bin/time -l`. O OpenClaw requer o runtime Node.js (tipicamente ~390MB de overhead adicional de memória), enquanto o NanoBot requer o runtime Python. PicoClaw e Hrafn são binários estáticos. Os valores de RAM acima são memória em runtime; os requisitos de compilação são maiores.
...
O Hrafn suporta perfis de autenticação nativos de assinatura (multi-conta, criptografados em repouso).

- Arquivo de armazenamento: `~/.hrafn/auth-profiles.json`
- Chave de criptografia: `~/.hrafn/.secret_key`
...
Raiz do workspace: `~/.hrafn/workspace/` (configurável via config).
...
Skills: `~/.hrafn/workspace/skills/<skill>/SKILL.md` ou `SKILL.toml`.
...
hrafn onboard              # Assistente de configuração guiada
hrafn status               # Mostrar status do daemon/agente
hrafn doctor               # Executar diagnósticos do sistema

# Gateway + daemon
hrafn gateway              # Iniciar servidor gateway (127.0.0.1:42617)
hrafn daemon               # Iniciar runtime autônomo completo

# Agente
hrafn agent                # Modo de chat interativo
hrafn agent -m "message"   # Modo de mensagem única

# Gerenciamento de serviços
hrafn service install      # Instalar como serviço do SO (launchd/systemd)
hrafn service start|stop|restart|status

# Canais
hrafn channel list         # Listar canais configurados
hrafn channel doctor       # Verificar saúde dos canais
hrafn channel bind-telegram 123456789

# Cron + agendamento
hrafn cron list            # Listar trabalhos agendados
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Memória
hrafn memory list          # Listar entradas de memória
hrafn memory get <key>     # Recuperar uma memória
hrafn memory stats         # Estatísticas de memória
...
hrafn peripheral flash     # Flashear firmware no dispositivo
...
Ou pule os passos acima e instale tudo (dependências do sistema, Rust, Hrafn) em um único comando:
...
- Siga o [Hrafn Onboard](#início-rápido) para uma configuração guiada.
...
O Hrafn foi construído para o caranguejo suave 🦀, um assistente de IA rápido e eficiente. Construído por Argenis De La Rosa e a comunidade.
...
Se o Hrafn ajuda no seu trabalho e você quer apoiar o desenvolvimento contínuo, pode doar aqui:
...
Novo no Hrafn? Procure issues rotulados como [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — veja nosso [Guia de contribuição](CONTRIBUTING.md#first-time-contributors) para saber como começar. PRs com IA/vibe-coded são bem-vindos! 🤖
...
- Novo `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**Este é o único repositório oficial do Hrafn:**

> https://github.com/5queezer/hrafn

Qualquer outro repositório, organização, domínio ou pacote que afirme ser "Hrafn" ou implique afiliação com Hrafn Labs **não é autorizado e não é afiliado a este projeto**. Forks não autorizados conhecidos serão listados em [TRADEMARK.md](docs/maintainers/trademark.md).

Se encontrar falsificação ou uso indevido de marca, por favor [abra um issue](https://github.com/5queezer/hrafn/issues).

---

## Licença

O Hrafn tem licença dupla para máxima abertura e proteção dos contribuidores:
...
O nome e logo do **Hrafn** são marcas registradas da Hrafn Labs. Esta licença não concede permissão para usá-los para implicar endosso ou afiliação. Veja [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos e proibidos.
...
**Hrafn** — Zero overhead. Zero compromisso. Implante em qualquer lugar. Troque qualquer coisa. 🦀
