...
<h1 align="center">🦀 Hrafn — Персональный ИИ-ассистент</h1>
...
<p align="center">
  <a href="https://hrafn.ai">Веб-сайт</a> ·
...
> **Рекомендуемая настройка:** выполните `hrafn onboard` в терминале. Hrafn Onboard пошагово проведёт вас через настройку gateway, рабочего пространства, каналов и провайдера. Это рекомендуемый путь настройки, работающий на macOS, Linux и Windows (через WSL2). Новая установка? Начните здесь: [Начало работы](#быстрый-старт)
...
`hrafn onboard` запускается автоматически после установки для настройки рабочего пространства и провайдера.
...
hrafn gateway --port 0       # random port (security hardened)
...
Обновляетесь? Выполните `hrafn doctor` после обновления.
...
Hrafn может импортировать ваше рабочее пространство, память и конфигурацию OpenClaw:
...
Это переносит ваши записи памяти, файлы рабочего пространства и конфигурацию из `~/.openclaw/` в `~/.hrafn/`. Конфигурация автоматически конвертируется из JSON в TOML.

## Настройки безопасности по умолчанию (доступ через ЛС)

Hrafn подключается к реальным поверхностям обмена сообщениями. Относитесь к входящим ЛС как к ненадёжному вводу.
...
- Одобрение через: `hrafn pairing approve <channel> <code>` (затем отправитель добавляется в локальный список разрешённых).
- Публичные входящие ЛС требуют явного включения в `config.toml`.
- Выполните `hrafn doctor` для выявления рискованных или неправильно настроенных политик ЛС.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критический_ | Мы **не связаны** с `openagen/hrafn`, `hrafn.org` или `hrafn.net`. Домены `hrafn.org` и `hrafn.net` в настоящее время указывают на форк `openagen/hrafn`, и этот домен/репозиторий выдают себя за наш официальный сайт/проект. | Не доверяйте информации, бинарным файлам, сбору средств или объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/5queezer/hrafn) и наши верифицированные аккаунты в социальных сетях. |
...
## Снимок бенчмарков (Hrafn vs OpenClaw, воспроизводимый)
...
> Примечания: результаты Hrafn измерены на релизных сборках с использованием `/usr/bin/time -l`. OpenClaw требует среду выполнения Node.js (обычно ~390 МБ дополнительных накладных расходов памяти), а NanoBot требует среду выполнения Python. PicoClaw и Hrafn — статические бинарные файлы. Показатели ОЗУ выше — это память времени выполнения; требования к компиляции при сборке выше.
...
Hrafn поддерживает нативные профили аутентификации по подписке (мультиаккаунт, шифрование в состоянии покоя).

- Файл хранилища: `~/.hrafn/auth-profiles.json`
- Ключ шифрования: `~/.hrafn/.secret_key`
...
Корень рабочего пространства: `~/.hrafn/workspace/` (настраивается через конфигурацию).
...
Навыки: `~/.hrafn/workspace/skills/<skill>/SKILL.md` или `SKILL.toml`.
...
hrafn status               # Show daemon/agent status
hrafn doctor               # Run system diagnostics

# Gateway + daemon
hrafn gateway              # Start gateway server (127.0.0.1:42617)
hrafn daemon               # Start full autonomous runtime
...
# Service management
hrafn service install      # Install as OS service (launchd/systemd)
hrafn service start|stop|restart|status

# Channels
hrafn channel list         # List configured channels
hrafn channel doctor       # Check channel health
hrafn channel bind-telegram 123456789

# Cron + scheduling
hrafn cron list            # List scheduled jobs
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Memory
hrafn memory list          # List memory entries
hrafn memory get <key>     # Retrieve a memory
hrafn memory stats         # Memory statistics
...
hrafn hardware discover    # Scan for connected devices
hrafn peripheral list      # List connected peripherals
hrafn peripheral flash     # Flash firmware to device
...
Или пропустите шаги выше и установите всё (системные зависимости, Rust, Hrafn) одной командой:
...
- Следуйте [Hrafn Onboard](#быстрый-старт) для управляемой настройки.
...
Hrafn был создан для smooth crab 🦀 — быстрого и эффективного ИИ-ассистента. Создан Argenis De La Rosa и сообществом.
...
Если Hrafn помогает вашей работе и вы хотите поддержать дальнейшую разработку, вы можете пожертвовать здесь:
...
Новичок в Hrafn? Ищите задачи с меткой [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — см. наше [Руководство по участию](CONTRIBUTING.md#first-time-contributors) для начала. AI/vibe-coded PR приветствуются! 🤖
...
- Новый `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**Это единственный официальный репозиторий Hrafn:**

> https://github.com/5queezer/hrafn

Любой другой репозиторий, организация, домен или пакет, претендующий на звание «Hrafn» или подразумевающий связь с Hrafn Labs, является **неавторизованным и не связанным с этим проектом**. Известные неавторизованные форки будут перечислены в [TRADEMARK.md](docs/maintainers/trademark.md).

Если вы столкнётесь с имитацией или неправомерным использованием товарного знака, пожалуйста, [откройте issue](https://github.com/5queezer/hrafn/issues).

---

## Лицензия

Hrafn распространяется под двойной лицензией для максимальной открытости и защиты участников:
...
Название и логотип **Hrafn** являются товарными знаками Hrafn Labs. Эта лицензия не предоставляет разрешения на их использование для подразумевания одобрения или принадлежности. См. [TRADEMARK.md](docs/maintainers/trademark.md) для разрешённых и запрещённых использований.
...
**Hrafn** — Нулевые накладные расходы. Нулевые компромиссы. Развёртывайте где угодно. Заменяйте что угодно. 🦀
