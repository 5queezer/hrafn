...
> **Rekommenderad konfiguration:** kör `hrafn onboard` i din terminal. Hrafn Onboard guidar dig steg för steg genom att konfigurera gateway, arbetsyta, kanaler och leverantör. Det är den rekommenderade installationsvägen och fungerar på macOS, Linux och Windows (via WSL2). Ny installation? Börja här: [Kom igång](#snabbstart)
...
`hrafn onboard` körs automatiskt efter installationen för att konfigurera din arbetsyta och leverantör.
...
hrafn gateway --port 0       # slumpmässig port (säkerhetshärdad)
...
Uppgraderar du? Kör `hrafn doctor` efter uppdatering.
...
Hrafn kan importera din OpenClaw-arbetsyta, minne och konfiguration:
...
Detta migrerar dina minnesposter, arbetsytefiler och konfiguration från `~/.openclaw/` till `~/.hrafn/`. Konfiguration konverteras automatiskt från JSON till TOML.

## Säkerhetsstandarder (DM-åtkomst)

Hrafn ansluter till riktiga meddelandeytor. Behandla inkommande DM som opålitlig indata.
...
- Godkänn med: `hrafn pairing approve <channel> <code>` (sedan läggs avsändaren till i en lokal tillåtlista).
- Offentliga inkommande DM kräver ett explicit opt-in i `config.toml`.
- Kör `hrafn doctor` för att hitta riskfyllda eller felkonfigurerade DM-policyer.
...
| ----------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19  | _Kritisk_   | Vi är **inte affilierade** med `openagen/hrafn`, `hrafn.org` eller `hrafn.net`. Domänerna `hrafn.org` och `hrafn.net` pekar för närvarande till `openagen/hrafn`-forken, och den domänen/repositoryt utger sig för att vara vår officiella webbplats/projekt.                                                                         | Lita inte på information, binärer, insamlingar eller meddelanden från dessa källor. Använd bara [detta repository](https://github.com/5queezer/hrafn) och våra verifierade sociala konton.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
## Benchmarkögonblicksbild (Hrafn vs OpenClaw, Reproducerbar)
...
> Noteringar: Hrafn-resultat mäts på release-byggen med `/usr/bin/time -l`. OpenClaw kräver Node.js-körmiljö (typiskt ~390MB extra minnesoverhead), medan NanoBot kräver Python-körmiljö. PicoClaw och Hrafn är statiska binärer. RAM-siffrorna ovan är körtidsminne; kompileringskrav vid byggtid är högre.

<p align="center">
  <img src="docs/assets/hrafn-comparison.jpeg" alt="Hrafn vs OpenClaw jämförelse" width="800" />
</p>
...
Hrafn stöder prenumerationsnativa autentiseringsprofiler (multikonto, krypterat i vila).

- Lagringsfil: `~/.hrafn/auth-profiles.json`
- Krypteringsnyckel: `~/.hrafn/.secret_key`
...
Arbetsyterot: `~/.hrafn/workspace/` (konfigurerbart via config).
...
Färdigheter: `~/.hrafn/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
```bash
# Arbetsytehantering
hrafn onboard              # Guidad installationsguide
hrafn status               # Visa daemon-/agentstatus
hrafn doctor               # Kör systemdiagnostik

# Gateway + daemon
hrafn gateway              # Starta gateway-server (127.0.0.1:42617)
hrafn daemon               # Starta full autonom körmiljö

# Agent
hrafn agent                # Interaktivt chattläge
hrafn agent -m "message"   # Enstaka meddelandeläge

# Tjänstehantering
hrafn service install      # Installera som OS-tjänst (launchd/systemd)
hrafn service start|stop|restart|status

# Kanaler
hrafn channel list         # Lista konfigurerade kanaler
hrafn channel doctor       # Kontrollera kanalhälsa
hrafn channel bind-telegram 123456789

# Cron + schemaläggning
hrafn cron list            # Lista schemalagda jobb
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Minne
hrafn memory list          # Lista minnesposter
hrafn memory get <key>     # Hämta ett minne
hrafn memory stats         # Minnesstatistik
...
hrafn hardware discover    # Sök efter anslutna enheter
hrafn peripheral list      # Lista anslutna periferienheter
hrafn peripheral flash     # Flasha firmware till enhet
...
Eller hoppa över stegen ovan och installera allt (systemberoenden, Rust, Hrafn) med ett enda kommando:
...
- Följ [Hrafn Onboard](#snabbstart) för en guidad installation.
...
Hrafn byggdes för smooth crab 🦀, en snabb och effektiv AI-assistent. Byggd av Argenis De La Rosa och gemenskapen.
...
Om Hrafn hjälper ditt arbete och du vill stödja pågående utveckling kan du donera här:
...
Ny till Hrafn? Leta efter ärenden märkta [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vår [Bidragsguide](CONTRIBUTING.md#first-time-contributors) för hur du kommer igång. AI/vibe-kodade PR:er är välkomna! 🤖
...
**Detta är det enda officiella Hrafn-repositoryt:**

> https://github.com/5queezer/hrafn

Alla andra repositorier, organisationer, domäner eller paket som hävdar att vara "Hrafn" eller antyder anslutning till Hrafn Labs är **obehöriga och inte affilierade med detta projekt**. Kända obehöriga forkar listas i [TRADEMARK.md](docs/maintainers/trademark.md).

Om du stöter på imitation eller varumärkesmissbruk, vänligen [öppna ett ärende](https://github.com/5queezer/hrafn/issues).

---

## Licens

Hrafn är dubbellicensierat för maximal öppenhet och bidragsgivarskydd:
...
**Hrafn**-namnet och logotypen är varumärken som tillhör Hrafn Labs. Denna licens beviljar inte tillstånd att använda dem för att antyda stöd eller anslutning. Se [TRADEMARK.md](docs/maintainers/trademark.md) för tillåtna och förbjudna användningar.
...
**Hrafn** — Noll overhead. Noll kompromiss. Distribuera var som helst. Byt ut vad som helst. 🦀

## Bidragsgivare

<a href="https://github.com/5queezer/hrafn/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=5queezer/hrafn" alt="Hrafn-bidragsgivare" />
</a>
