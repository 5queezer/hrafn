...
<h1 align="center">🦀 Hrafn — ผู้ช่วย AI ส่วนตัว</h1>
...
> **การตั้งค่าที่แนะนำ:** รัน `hrafn onboard` ในเทอร์มินัลของคุณ Hrafn Onboard จะแนะนำคุณทีละขั้นตอนในการตั้งค่า gateway, workspace, ช่องทาง และ provider เป็นเส้นทางการตั้งค่าที่แนะนำและใช้งานได้บน macOS, Linux และ Windows (ผ่าน WSL2) ติดตั้งใหม่? เริ่มที่นี่: [เริ่มต้นใช้งาน](#เริ่มต้นอย่างรวดเร็ว)
...
`hrafn onboard` จะรันโดยอัตโนมัติหลังติดตั้งเพื่อกำหนดค่า workspace และ provider ของคุณ
...
hrafn gateway                # ค่าเริ่มต้น: 127.0.0.1:42617
hrafn gateway --port 0       # พอร์ตสุ่ม (ความปลอดภัยเพิ่มขึ้น)
...
กำลังอัปเกรด? รัน `hrafn doctor` หลังจากอัปเดต
...
Hrafn สามารถนำเข้า workspace, หน่วยความจำ และการกำหนดค่าจาก OpenClaw ของคุณ:
...
สิ่งนี้จะย้ายรายการหน่วยความจำ ไฟล์ workspace และการกำหนดค่าจาก `~/.openclaw/` ไปยัง `~/.hrafn/` การกำหนดค่าจะถูกแปลงจาก JSON เป็น TOML โดยอัตโนมัติ

## ค่าเริ่มต้นด้านความปลอดภัย (การเข้าถึง DM)

Hrafn เชื่อมต่อกับพื้นผิวการส่งข้อความจริง ถือว่า DM ขาเข้าเป็นข้อมูลที่ไม่น่าเชื่อถือ
...
- อนุมัติด้วย: `hrafn pairing approve <channel> <code>` (จากนั้นผู้ส่งจะถูกเพิ่มในรายการอนุญาตในเครื่อง)
- DM ขาเข้าสาธารณะต้องมีการเลือกเข้าร่วมอย่างชัดเจนใน `config.toml`
- รัน `hrafn doctor` เพื่อค้นหานโยบาย DM ที่เสี่ยงหรือกำหนดค่าผิด
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _วิกฤต_  | เรา**ไม่มีส่วนเกี่ยวข้อง**กับ `openagen/hrafn`, `hrafn.org` หรือ `hrafn.net` โดเมน `hrafn.org` และ `hrafn.net` ปัจจุบันชี้ไปที่ fork `openagen/hrafn` และโดเมน/repository เหล่านั้นกำลังปลอมตัวเป็นเว็บไซต์/โปรเจกต์อย่างเป็นทางการของเรา                                                                                       | อย่าเชื่อถือข้อมูล ไบนารี การระดมทุน หรือประกาศจากแหล่งเหล่านั้น ใช้เฉพาะ[repository นี้](https://github.com/5queezer/hrafn)และบัญชีโซเชียลที่ได้รับการยืนยันของเรา                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## สรุป Benchmark (Hrafn vs OpenClaw, ทำซ้ำได้)
...
> หมายเหตุ: ผลลัพธ์ Hrafn วัดจาก release builds โดยใช้ `/usr/bin/time -l` OpenClaw ต้องการ runtime Node.js (โดยทั่วไป ~390MB overhead หน่วยความจำเพิ่มเติม) ในขณะที่ NanoBot ต้องการ runtime Python PicoClaw และ Hrafn เป็นไบนารีแบบ static ตัวเลข RAM ด้านบนเป็นหน่วยความจำ runtime ความต้องการการคอมไพล์ตอน build สูงกว่า
...
Hrafn รองรับโปรไฟล์การยืนยันตัวตนแบบ subscription-native (หลายบัญชี, เข้ารหัสเมื่อเก็บ)

- ไฟล์จัดเก็บ: `~/.hrafn/auth-profiles.json`
- คีย์เข้ารหัส: `~/.hrafn/.secret_key`
...
Root workspace: `~/.hrafn/workspace/` (กำหนดค่าได้ผ่าน config)
...
Skills: `~/.hrafn/workspace/skills/<skill>/SKILL.md` หรือ `SKILL.toml`
...
```bash
# การจัดการ workspace
hrafn onboard              # วิซาร์ดการตั้งค่าแบบแนะนำ
hrafn status               # แสดงสถานะ daemon/เอเจนต์
hrafn doctor               # รันการวินิจฉัยระบบ

# Gateway + daemon
hrafn gateway              # เริ่มเซิร์ฟเวอร์ gateway (127.0.0.1:42617)
hrafn daemon               # เริ่ม runtime อัตโนมัติเต็มรูปแบบ

# เอเจนต์
hrafn agent                # โหมดแชทโต้ตอบ
hrafn agent -m "message"   # โหมดข้อความเดียว

# การจัดการบริการ
hrafn service install      # ติดตั้งเป็นบริการ OS (launchd/systemd)
hrafn service start|stop|restart|status

# ช่องทาง
hrafn channel list         # แสดงรายการช่องทางที่กำหนดค่า
hrafn channel doctor       # ตรวจสอบสุขภาพช่องทาง
hrafn channel bind-telegram 123456789

# Cron + การกำหนดเวลา
hrafn cron list            # แสดงรายการงานที่กำหนดเวลา
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# หน่วยความจำ
hrafn memory list          # แสดงรายการหน่วยความจำ
hrafn memory get <key>     # ดึงหน่วยความจำ
hrafn memory stats         # สถิติหน่วยความจำ
...
# อุปกรณ์ต่อพ่วง
hrafn hardware discover    # สแกนอุปกรณ์ที่เชื่อมต่อ
hrafn peripheral list      # แสดงรายการอุปกรณ์ต่อพ่วงที่เชื่อมต่อ
hrafn peripheral flash     # แฟลช firmware ไปยังอุปกรณ์
...
หรือข้ามขั้นตอนด้านบนและติดตั้งทุกอย่าง (dependencies ระบบ, Rust, Hrafn) ในคำสั่งเดียว:
...
- ทำตาม [Hrafn Onboard](#เริ่มต้นอย่างรวดเร็ว) สำหรับการตั้งค่าแบบแนะนำ
...
Hrafn ถูกสร้างสำหรับ smooth crab 🦀 ผู้ช่วย AI ที่เร็วและมีประสิทธิภาพ สร้างโดย Argenis De La Rosa และชุมชน
...
หาก Hrafn ช่วยงานของคุณและคุณต้องการสนับสนุนการพัฒนาต่อเนื่อง คุณสามารถบริจาคที่นี่:
...
ใหม่กับ Hrafn? มองหา issues ที่มีป้ายกำกับ [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ดู[คู่มือการมีส่วนร่วม](CONTRIBUTING.md#first-time-contributors)สำหรับวิธีเริ่มต้น ยินดีรับ PR ที่สร้างด้วย AI/vibe-coded! 🤖
...
- `Skill` ใหม่ → `~/.hrafn/workspace/skills/<name>/`
...
**นี่คือ repository อย่างเป็นทางการเพียงแห่งเดียวของ Hrafn:**

> https://github.com/5queezer/hrafn

repository, องค์กร, โดเมน หรือแพ็กเกจอื่นใดที่อ้างว่าเป็น "Hrafn" หรือบ่งบอกถึงการเกี่ยวข้องกับ Hrafn Labs นั้น**ไม่ได้รับอนุญาตและไม่มีส่วนเกี่ยวข้องกับโปรเจกต์นี้** Fork ที่ไม่ได้รับอนุญาตที่ทราบจะถูกระบุไว้ใน [TRADEMARK.md](docs/maintainers/trademark.md)

หากคุณพบการแอบอ้างหรือการใช้เครื่องหมายการค้าในทางที่ผิด โปรด[เปิด issue](https://github.com/5queezer/hrafn/issues)

---

## สัญญาอนุญาต

Hrafn มี dual-license เพื่อความเปิดกว้างสูงสุดและการปกป้องผู้มีส่วนร่วม:
...
ชื่อและโลโก้ **Hrafn** เป็นเครื่องหมายการค้าของ Hrafn Labs สัญญาอนุญาตนี้ไม่ให้สิทธิ์ในการใช้เพื่อบ่งบอกถึงการรับรองหรือการเกี่ยวข้อง ดู [TRADEMARK.md](docs/maintainers/trademark.md) สำหรับการใช้งานที่อนุญาตและห้าม
...
**Hrafn** — ไม่มีโอเวอร์เฮด ไม่มีการประนีประนอม Deploy ที่ไหนก็ได้ สลับอะไรก็ได้ 🦀
