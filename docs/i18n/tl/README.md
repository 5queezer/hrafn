...
<h1 align="center">🦀 Hrafn — Personal na AI Assistant</h1>
...
> **Inirerekomendang setup:** patakbuhin ang `hrafn onboard` sa iyong terminal. Ang Hrafn Onboard ay gagabay sa iyo hakbang-hakbang sa pag-setup ng gateway, workspace, channel, at provider. Ito ang inirerekomendang setup path at gumagana sa macOS, Linux, at Windows (sa pamamagitan ng WSL2). Bagong install? Magsimula dito: [Magsimula](#mabilis-na-simula-tldr)
...
Awtomatikong tatakbo ang `hrafn onboard` pagkatapos ng install para i-configure ang iyong workspace at provider.
...
hrafn gateway                # default: 127.0.0.1:42617
hrafn gateway --port 0       # random port (pinalakas na seguridad)
...
Nag-upgrade? Patakbuhin ang `hrafn doctor` pagkatapos mag-update.
...
Maaaring i-import ng Hrafn ang iyong OpenClaw workspace, memory, at configuration:
...
Inililipat nito ang iyong memory entries, workspace files, at configuration mula `~/.openclaw/` patungo sa `~/.hrafn/`. Awtomatikong kino-convert ang config mula JSON patungong TOML.

## Mga default sa seguridad (DM access)

Kumokonekta ang Hrafn sa totoong mga messaging surface. Tratuhin ang mga papasok na DM bilang hindi mapagkakatiwalaang input.
...
- I-approve gamit ang: `hrafn pairing approve <channel> <code>` (pagkatapos ay idadagdag ang nagpadala sa lokal na allowlist).
- Ang mga pampublikong papasok na DM ay nangangailangan ng tahasang opt-in sa `config.toml`.
- Patakbuhin ang `hrafn doctor` para makita ang mga mapanganib o maling naka-configure na DM policy.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritikal_ | **Hindi kami konektado** sa `openagen/hrafn`, `hrafn.org` o `hrafn.net`. Ang `hrafn.org` at `hrafn.net` na mga domain ay kasalukuyang nakaturo sa `openagen/hrafn` fork, at ang domain/repository na iyon ay nanggagaya sa aming opisyal na website/proyekto. | Huwag magtiwala sa impormasyon, binaries, fundraising, o mga anunsyo mula sa mga pinagmulang iyon. Gamitin lamang [ang repository na ito](https://github.com/5queezer/hrafn) at ang aming mga verified na social account. |
...
### Bakit pinipili ng mga team ang Hrafn
...
## Benchmark Snapshot (Hrafn vs OpenClaw, Reproducible)
...
> Mga Tala: Ang mga resulta ng Hrafn ay sinusukat sa release builds gamit ang `/usr/bin/time -l`. Ang OpenClaw ay nangangailangan ng Node.js runtime (karaniwang ~390MB dagdag na memory overhead), habang ang NanoBot ay nangangailangan ng Python runtime. Ang PicoClaw at Hrafn ay static binaries. Ang mga RAM figure sa itaas ay runtime memory; ang build-time compilation requirements ay mas mataas.
...
Sinusuportahan ng Hrafn ang subscription-native auth profiles (multi-account, encrypted at rest).

- Store file: `~/.hrafn/auth-profiles.json`
- Encryption key: `~/.hrafn/.secret_key`
...
Workspace root: `~/.hrafn/workspace/` (configurable sa pamamagitan ng config).
...
Skills: `~/.hrafn/workspace/skills/<skill>/SKILL.md` o `SKILL.toml`.
...
hrafn onboard              # Guided setup wizard
hrafn status               # Ipakita ang daemon/agent status
hrafn doctor               # Patakbuhin ang system diagnostics

# Gateway + daemon
hrafn gateway              # Simulan ang gateway server (127.0.0.1:42617)
hrafn daemon               # Simulan ang buong autonomous runtime

# Agent
hrafn agent                # Interactive chat mode
hrafn agent -m "message"   # Single message mode

# Service management
hrafn service install      # I-install bilang OS service (launchd/systemd)
hrafn service start|stop|restart|status

# Mga channel
hrafn channel list         # Ilista ang mga configured na channel
hrafn channel doctor       # Suriin ang kalusugan ng channel
hrafn channel bind-telegram 123456789

# Cron + scheduling
hrafn cron list            # Ilista ang mga naka-schedule na gawain
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Memory
hrafn memory list          # Ilista ang mga memory entry
hrafn memory get <key>     # Kunin ang isang memory
hrafn memory stats         # Estadistika ng memory
...
hrafn hardware discover    # I-scan ang mga konektadong device
hrafn peripheral list      # Ilista ang mga konektadong peripheral
hrafn peripheral flash     # I-flash ang firmware sa device
...
O laktawan ang mga hakbang sa itaas at i-install ang lahat (system deps, Rust, Hrafn) sa isang command:
...
- Sundin ang [Hrafn Onboard](#mabilis-na-simula-tldr) para sa guided setup.
...
Ang Hrafn ay binuo para sa smooth crab 🦀, isang mabilis at mahusay na AI assistant. Binuo ni Argenis De La Rosa at ng komunidad.

- [hrafn.ai](https://hrafn.ai)
- [@hrafn](https://x.com/hrafn)

## Suportahan ang Hrafn

Kung nakakatulong ang Hrafn sa iyong trabaho at gusto mong suportahan ang patuloy na development, maaari kang mag-donate dito:
...
Bago sa Hrafn? Hanapin ang mga issue na may label na [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — tingnan ang aming [Contributing Guide](CONTRIBUTING.md#first-time-contributors) kung paano magsimula. Ang AI/vibe-coded PRs ay welcome! 🤖
...
- Bagong `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**Ito ang tanging opisyal na Hrafn repository:**

> https://github.com/5queezer/hrafn

Ang anumang iba pang repository, organisasyon, domain, o package na nag-aangkin na "Hrafn" o nagpapahiwatig ng affiliation sa Hrafn Labs ay **hindi awtorisado at hindi konektado sa proyektong ito**. Ang mga kilalang unauthorized forks ay ililista sa [TRADEMARK.md](docs/maintainers/trademark.md).

Kung makakita ka ng panggagaya o trademark misuse, mangyaring [mag-open ng issue](https://github.com/5queezer/hrafn/issues).

---

## Lisensya

Ang Hrafn ay dual-licensed para sa maximum na openness at proteksyon ng contributor:
...
Ang pangalang **Hrafn** at logo ay mga trademark ng Hrafn Labs. Ang lisensyang ito ay hindi nagbibigay ng pahintulot na gamitin ang mga ito upang ipahiwatig ang endorsement o affiliation. Tingnan ang [TRADEMARK.md](docs/maintainers/trademark.md) para sa mga pinapahintulutan at ipinagbabawal na gamit.
...
**Hrafn** — Zero overhead. Zero kompromiso. I-deploy kahit saan. I-swap ang kahit ano. 🦀
