...
<h1 align="center">🦀 Hrafn — Персональний AI-Асистент</h1>
...
`hrafn onboard` запускається автоматично після встановлення для налаштування вашого робочого простору та провайдера.
...
hrafn gateway                # за замовчуванням: 127.0.0.1:42617
hrafn gateway --port 0       # випадковий порт (посилена безпека)
...
Оновлюєтесь? Виконайте `hrafn doctor` після оновлення.
...
Hrafn може імпортувати ваш робочий простір, пам'ять та конфігурацію OpenClaw:
...
Це мігрує ваші записи пам'яті, файли робочого простору та конфігурацію з `~/.openclaw/` до `~/.hrafn/`. Конфігурація автоматично конвертується з JSON у TOML.

## Стандартні налаштування безпеки (доступ через DM)

Hrafn підключається до реальних платформ обміну повідомленнями. Розглядайте вхідні DM як ненадійний ввід.
...
- Підтвердіть за допомогою: `hrafn pairing approve <channel> <code>` (після чого відправник додається до локального списку дозволених).
- Публічні вхідні DM вимагають явного увімкнення в `config.toml`.
- Виконайте `hrafn doctor` для виявлення ризикованих або неправильно налаштованих політик DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критичний_ | Ми **не пов'язані** з `openagen/hrafn`, `hrafn.org` або `hrafn.net`. Домени `hrafn.org` та `hrafn.net` наразі вказують на форк `openagen/hrafn`, і цей домен/репозиторій видають себе за наш офіційний вебсайт/проєкт. | Не довіряйте інформації, бінарним файлам, збору коштів або оголошенням з цих джерел. Використовуйте лише [цей репозиторій](https://github.com/5queezer/hrafn) та наші верифіковані соціальні акаунти. |
...
## Порівняльний бенчмарк (Hrafn проти OpenClaw, відтворюваний)
...
> Примітки: результати Hrafn виміряні на релізних збірках за допомогою `/usr/bin/time -l`. OpenClaw вимагає середовище Node.js (зазвичай ~390MB додаткових накладних витрат пам'яті), тоді як NanoBot вимагає середовище Python. PicoClaw і Hrafn — це статичні бінарні файли. Наведені цифри RAM — це пам'ять часу виконання; вимоги до компіляції вищі.
...
Hrafn підтримує профілі авторизації на основі підписки (мультиакаунт, шифрування в стані спокою).

- Файл сховища: `~/.hrafn/auth-profiles.json`
- Ключ шифрування: `~/.hrafn/.secret_key`
...
Корінь робочого простору: `~/.hrafn/workspace/` (налаштовується через конфігурацію).
...
Навички: `~/.hrafn/workspace/skills/<skill>/SKILL.md` або `SKILL.toml`.
...
hrafn onboard              # Покроковий майстер налаштування
hrafn status               # Показати стан демона/агента
hrafn doctor               # Запустити діагностику системи

# Gateway + демон
hrafn gateway              # Запустити сервер gateway (127.0.0.1:42617)
hrafn daemon               # Запустити повне автономне середовище

# Агент
hrafn agent                # Інтерактивний режим чату
hrafn agent -m "message"   # Режим одного повідомлення

# Керування сервісом
hrafn service install      # Встановити як системний сервіс (launchd/systemd)
hrafn service start|stop|restart|status

# Канали
hrafn channel list         # Список налаштованих каналів
hrafn channel doctor       # Перевірка стану каналів
hrafn channel bind-telegram 123456789

# Cron + планування
hrafn cron list            # Список запланованих завдань
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Пам'ять
hrafn memory list          # Список записів пам'яті
hrafn memory get <key>     # Отримати запис пам'яті
hrafn memory stats         # Статистика пам'яті
...
hrafn hardware discover    # Сканування підключених пристроїв
hrafn peripheral list      # Список підключених периферійних пристроїв
hrafn peripheral flash     # Прошивка пристрою
...
Або пропустіть кроки вище і встановіть все (системні залежності, Rust, Hrafn) однією командою:
...
- Слідуйте [Hrafn Onboard](#швидкий-старт-tldr) для покрокового налаштування.
...
Hrafn створений для smooth crab 🦀, швидкого та ефективного AI-асистента. Створений Argenis De La Rosa та спільнотою.
...
Якщо Hrafn допомагає вашій роботі і ви хочете підтримати подальшу розробку, ви можете зробити пожертву тут:
...
Новачок у Hrafn? Шукайте завдання з міткою [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — дивіться наш [Посібник з внеску](CONTRIBUTING.md#first-time-contributors) для початку. PR з AI-допомогою вітаються!
...
- Новий `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**Це єдиний офіційний репозиторій Hrafn:**

> https://github.com/5queezer/hrafn

Будь-який інший репозиторій, організація, домен або пакет, що претендує на назву "Hrafn" або натякає на зв'язок з Hrafn Labs, є **неавторизованим і не пов'язаним з цим проєктом**. Відомі неавторизовані форки перелічені в [TRADEMARK.md](docs/maintainers/trademark.md).

Якщо ви зіткнулися з імітацією або зловживанням торговою маркою, будь ласка, [створіть issue](https://github.com/5queezer/hrafn/issues).

---

## Ліцензія

Hrafn має подвійну ліцензію для максимальної відкритості та захисту учасників:
...
Назва та логотип **Hrafn** є торговими марками Hrafn Labs. Ця ліцензія не надає дозволу використовувати їх для підтвердження або зв'язку. Дивіться [TRADEMARK.md](docs/maintainers/trademark.md) для дозволених та заборонених використань.
...
**Hrafn** — Нуль накладних витрат. Нуль компромісів. Розгортайте будь-де. Замінюйте будь-що. 🦀
