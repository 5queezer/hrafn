...
<h1 align="center">🦀 Hrafn — ذاتی AI اسسٹنٹ</h1>
...
> **تجویز کردہ سیٹ اپ:** اپنے ٹرمینل میں `hrafn onboard` چلائیں۔ Hrafn Onboard آپ کو gateway، workspace، چینلز، اور provider ترتیب دینے میں مرحلہ وار رہنمائی کرتا ہے۔ یہ تجویز کردہ سیٹ اپ راستہ ہے اور macOS، Linux، اور Windows (WSL2 کے ذریعے) پر کام کرتا ہے۔ نئی تنصیب؟ یہاں سے شروع کریں: [شروع کریں](#فوری-آغاز)
...
`hrafn onboard` انسٹال کے بعد خود بخود چلتا ہے تاکہ آپ کا workspace اور provider ترتیب دیا جا سکے۔
...
hrafn gateway                # ڈیفالٹ: 127.0.0.1:42617
hrafn gateway --port 0       # بے ترتیب پورٹ (سیکیورٹی مضبوط)
...
اپ گریڈ کر رہے ہیں؟ اپ ڈیٹ کے بعد `hrafn doctor` چلائیں۔
...
Hrafn آپ کا OpenClaw workspace، میموری، اور کنفیگریشن درآمد کر سکتا ہے:
...
یہ آپ کے میموری اندراجات، workspace فائلیں، اور کنفیگریشن `~/.openclaw/` سے `~/.hrafn/` میں منتقل کرتا ہے۔ کنفیگ خود بخود JSON سے TOML میں تبدیل ہو جاتی ہے۔

## سیکیورٹی ڈیفالٹس (DM رسائی)

Hrafn حقیقی پیغام رسانی سطحوں سے جڑتا ہے۔ آنے والے DMs کو غیر بھروسہ مند ان پٹ سمجھیں۔
...
- منظوری دیں: `hrafn pairing approve <channel> <code>` (پھر بھیجنے والا مقامی اجازت نامہ میں شامل ہو جاتا ہے)۔
- عوامی آنے والے DMs کے لیے `config.toml` میں واضح opt-in ضروری ہے۔
- خطرناک یا غلط ترتیب شدہ DM پالیسیوں کا پتہ لگانے کے لیے `hrafn doctor` چلائیں۔
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _اہم ترین_  | ہم `openagen/hrafn`، `hrafn.org` یا `hrafn.net` سے **وابستہ نہیں** ہیں۔ `hrafn.org` اور `hrafn.net` ڈومینز فی الحال `openagen/hrafn` فورک کی طرف اشارہ کرتے ہیں، اور وہ ڈومین/ریپوزٹری ہماری سرکاری ویب سائٹ/پروجیکٹ کی نقل کر رہے ہیں۔                                                                                       | ان ذرائع سے معلومات، بائنریز، فنڈ ریزنگ، یا اعلانات پر بھروسہ نہ کریں۔ صرف [یہ ریپوزٹری](https://github.com/5queezer/hrafn) اور ہمارے تصدیق شدہ سوشل اکاؤنٹس استعمال کریں۔                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## بینچ مارک سنیپ شاٹ (Hrafn بمقابلہ OpenClaw، قابل تکرار)
...
Hrafn سبسکرپشن نیٹو auth پروفائلز سپورٹ کرتا ہے (ملٹی اکاؤنٹ، آرام پر خفیہ)۔

- اسٹور فائل: `~/.hrafn/auth-profiles.json`
- خفیہ کاری کلید: `~/.hrafn/.secret_key`
...
Workspace روٹ: `~/.hrafn/workspace/` (config کے ذریعے قابل ترتیب)۔
...
اسکلز: `~/.hrafn/workspace/skills/<skill>/SKILL.md` یا `SKILL.toml`۔
...
```bash
# Workspace مینجمنٹ
hrafn onboard              # رہنمائی شدہ سیٹ اپ وزرڈ
hrafn status               # daemon/ایجنٹ اسٹیٹس دکھائیں
hrafn doctor               # سسٹم تشخیص چلائیں

# Gateway + daemon
hrafn gateway              # Gateway سرور شروع کریں (127.0.0.1:42617)
hrafn daemon               # مکمل خودمختار رن ٹائم شروع کریں

# ایجنٹ
hrafn agent                # انٹرایکٹو چیٹ موڈ
hrafn agent -m "message"   # واحد پیغام موڈ

# سروس مینجمنٹ
hrafn service install      # OS سروس کے طور پر انسٹال کریں (launchd/systemd)
hrafn service start|stop|restart|status

# چینلز
hrafn channel list         # ترتیب شدہ چینلز کی فہرست
hrafn channel doctor       # چینل صحت چیک کریں
hrafn channel bind-telegram 123456789

# Cron + شیڈولنگ
hrafn cron list            # شیڈولڈ جابز کی فہرست
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# میموری
hrafn memory list          # میموری اندراجات کی فہرست
hrafn memory get <key>     # میموری حاصل کریں
hrafn memory stats         # میموری اعداد و شمار
...
# ہارڈویئر پیری فیرلز
hrafn hardware discover    # منسلک آلات اسکین کریں
hrafn peripheral list      # منسلک پیری فیرلز کی فہرست
hrafn peripheral flash     # آلے پر فرم ویئر فلیش کریں
...
یا اوپر کے مراحل چھوڑیں اور سب کچھ (سسٹم dependencies، Rust، Hrafn) ایک کمانڈ میں انسٹال کریں:
...
- رہنمائی شدہ سیٹ اپ کے لیے [Hrafn Onboard](#فوری-آغاز) فالو کریں۔
...
Hrafn smooth crab 🦀 کے لیے بنایا گیا تھا، ایک تیز اور مؤثر AI اسسٹنٹ۔ Argenis De La Rosa اور کمیونٹی نے بنایا۔
...
اگر Hrafn آپ کے کام میں مدد کرتا ہے اور آپ جاری ترقی کی حمایت کرنا چاہتے ہیں، تو آپ یہاں عطیہ دے سکتے ہیں:
...
Hrafn میں نئے ہیں؟ [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) لیبل والے issues تلاش کریں — شروع کرنے کے طریقے کے لیے [شراکت گائیڈ](CONTRIBUTING.md#first-time-contributors) دیکھیں۔ AI/vibe-coded PRs کا خیرمقدم ہے! 🤖
...
- نیا `Skill` → `~/.hrafn/workspace/skills/<name>/`
...
**یہ Hrafn کی واحد سرکاری ریپوزٹری ہے:**

> https://github.com/5queezer/hrafn

کوئی بھی دوسری ریپوزٹری، تنظیم، ڈومین، یا پیکیج جو "Hrafn" ہونے کا دعویٰ کرے یا Hrafn Labs سے وابستگی کا اشارہ کرے **غیر مجاز ہے اور اس پروجیکٹ سے وابستہ نہیں ہے**۔ معلوم غیر مجاز فورکس [TRADEMARK.md](docs/maintainers/trademark.md) میں درج ہوں گے۔

اگر آپ کو نقل یا ٹریڈ مارک کا غلط استعمال ملے، براہ کرم [issue کھولیں](https://github.com/5queezer/hrafn/issues)۔

---

## لائسنس

Hrafn زیادہ سے زیادہ کشادگی اور شراکت دار تحفظ کے لیے دوہری لائسنس یافتہ ہے:
...
**Hrafn** نام اور لوگو Hrafn Labs کے ٹریڈ مارکس ہیں۔ یہ لائسنس انہیں توثیق یا وابستگی کا اشارہ دینے کے لیے استعمال کرنے کی اجازت نہیں دیتا۔ مجاز اور ممنوع استعمال کے لیے [TRADEMARK.md](docs/maintainers/trademark.md) دیکھیں۔
...
**Hrafn** — صفر اوور ہیڈ۔ صفر سمجھوتا۔ کہیں بھی تعینات کریں۔ کچھ بھی تبدیل کریں۔ 🦀
