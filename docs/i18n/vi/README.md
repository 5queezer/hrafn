...
<h1 align="center">🦀 Hrafn — Trợ lý AI Cá nhân</h1>
...
> **Cài đặt khuyến nghị:** chạy `hrafn onboard` trong terminal. Hrafn Onboard hướng dẫn bạn từng bước thiết lập gateway, workspace, kênh và provider. Đây là đường dẫn cài đặt được khuyến nghị và hoạt động trên macOS, Linux, và Windows (qua WSL2). Cài đặt mới? Bắt đầu tại đây: [Bắt đầu](#bắt-đầu-nhanh-tldr)
...
`hrafn onboard` tự động chạy sau khi cài đặt để cấu hình workspace và provider.
...
hrafn gateway                # mặc định: 127.0.0.1:42617
hrafn gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
Đang nâng cấp? Chạy `hrafn doctor` sau khi cập nhật.
...
Hrafn có thể nhập workspace, bộ nhớ và cấu hình OpenClaw của bạn:
...
Thao tác này chuyển đổi các mục bộ nhớ, file workspace và cấu hình từ `~/.openclaw/` sang `~/.hrafn/`. Cấu hình được tự động chuyển từ JSON sang TOML.

## Mặc định bảo mật (truy cập DM)

Hrafn kết nối với các dịch vụ nhắn tin thực. Xem DM đến như đầu vào không đáng tin cậy.
...
- Phê duyệt bằng: `hrafn pairing approve <channel> <code>` (người gửi được thêm vào danh sách cho phép cục bộ).
- DM đến công khai yêu cầu opt-in rõ ràng trong `config.toml`.
- Chạy `hrafn doctor` để phát hiện chính sách DM nguy hiểm hoặc cấu hình sai.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không liên kết** với `openagen/hrafn`, `hrafn.org` hay `hrafn.net`. Các tên miền `hrafn.org` và `hrafn.net` hiện đang trỏ đến fork `openagen/hrafn`, và các tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/5queezer/hrafn) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
...
## So sánh hiệu năng (Hrafn vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả Hrafn được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), NanoBot yêu cầu runtime Python. PicoClaw và Hrafn là static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build cao hơn.
...
Hrafn hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.hrafn/auth-profiles.json`
- Khóa mã hóa: `~/.hrafn/.secret_key`
...
Thư mục gốc workspace: `~/.hrafn/workspace/` (có thể cấu hình qua config).
...
Skill: `~/.hrafn/workspace/skills/<skill>/SKILL.md` hoặc `SKILL.toml`.
...
```bash
# Quản lý workspace
hrafn onboard              # Trình hướng dẫn cài đặt
hrafn status               # Hiển thị trạng thái daemon/agent
hrafn doctor               # Chạy chẩn đoán hệ thống

# Gateway + daemon
hrafn gateway              # Khởi động gateway server (127.0.0.1:42617)
hrafn daemon               # Khởi động runtime tự trị đầy đủ

# Agent
hrafn agent                # Chế độ chat tương tác
hrafn agent -m "message"   # Chế độ tin nhắn đơn

# Quản lý dịch vụ
hrafn service install      # Cài đặt làm dịch vụ OS (launchd/systemd)
hrafn service start|stop|restart|status

# Kênh
hrafn channel list         # Liệt kê kênh đã cấu hình
hrafn channel doctor       # Kiểm tra sức khỏe kênh
hrafn channel bind-telegram 123456789

# Cron + lên lịch
hrafn cron list            # Liệt kê tác vụ đã lên lịch
hrafn cron add "*/5 * * * *" --prompt "Check system health"
hrafn cron remove <id>

# Bộ nhớ
hrafn memory list          # Liệt kê mục bộ nhớ
hrafn memory get <key>     # Truy xuất bộ nhớ
hrafn memory stats         # Thống kê bộ nhớ
...
hrafn hardware discover    # Quét thiết bị đã kết nối
hrafn peripheral list      # Liệt kê thiết bị ngoại vi đã kết nối
hrafn peripheral flash     # Flash firmware vào thiết bị
...
Hoặc bỏ qua các bước trên và cài hết mọi thứ (system deps, Rust, Hrafn) bằng một lệnh:
...
- Theo [Hrafn Onboard](#bắt-đầu-nhanh-tldr) để cài đặt có hướng dẫn.
...
Hrafn được xây dựng cho smooth crab 🦀, một trợ lý AI nhanh và hiệu quả. Được xây dựng bởi Argenis De La Rosa và cộng đồng.
...
Nếu Hrafn giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển, bạn có thể quyên góp tại đây:
...
Mới với Hrafn? Tìm các issue có nhãn [`good first issue`](https://github.com/5queezer/hrafn/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — xem [Hướng dẫn đóng góp](CONTRIBUTING.md#first-time-contributors) để bắt đầu. PR AI/vibe-coded đều được chào đón! 🤖
...
- `Skill` mới → `~/.hrafn/workspace/skills/<name>/`
...
**Đây là repository Hrafn chính thức duy nhất:**

> https://github.com/5queezer/hrafn

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "Hrafn" hoặc ngụ ý liên kết với Hrafn Labs đều **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/5queezer/hrafn/issues).

---

## Giấy phép

Hrafn được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **Hrafn** và logo là nhãn hiệu của Hrafn Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
**Hrafn** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
