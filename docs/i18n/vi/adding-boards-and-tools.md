# Thêm Board và Tool — Hướng dẫn phần cứng Hrafn

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào Hrafn.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.hrafn/config.toml)
hrafn peripheral add nucleo-f401re /dev/ttyACM0
hrafn peripheral add arduino-uno /dev/cu.usbmodem12345
hrafn peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, Hrafn có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `hrafn peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `hrafn peripheral list` | Liệt kê các board đã cấu hình |
| `hrafn peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `hrafn peripheral flash` | Nạp firmware Arduino |
| `hrafn peripheral flash-nucleo` | Nạp firmware Nucleo |
| `hrafn hardware discover` | Liệt kê thiết bị USB |
| `hrafn hardware info` | Thông tin chip qua probe-rs |
