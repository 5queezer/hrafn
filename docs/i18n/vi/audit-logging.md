...
## Vấn đề
Hrafn ghi log các hành động nhưng thiếu audit trail chống giả mạo cho:
...
# Chống giả mạo
sign_events = true
signing_key_path = "~/.config/hrafn/audit.key"
...
hrafn audit --user @alice

# Hiển thị tất cả lệnh rủi ro cao
hrafn audit --risk high

# Hiển thị vi phạm trong 24 giờ qua
hrafn audit --since 24h --violations-only

# Xuất sang JSON để phân tích
hrafn audit --format json --output audit.json
