...
[INFO] ✓ Audit logging enabled (~/.config/hrafn/audit.log)
...
$ hrafn security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/hrafn/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ hrafn config set security.sandbox.enabled false

# Bật backend cụ thể
$ hrafn config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ hrafn config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `hrafn security --status`
