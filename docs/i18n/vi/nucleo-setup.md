# Hrafn trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy Hrafn trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

Hrafn có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
Hrafn bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `hrafn peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
```bash
hrafn agent --message "Turn on the LED on pin 13"
```
...
| 3 | `hrafn peripheral flash-nucleo` |
| 4 | Thêm Nucleo vào config.toml (path = serial port của bạn) |
| 5 | `hrafn daemon` hoặc `hrafn agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `hrafn peripheral list` để xác nhận.
