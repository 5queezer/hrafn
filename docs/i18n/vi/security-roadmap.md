...
Hrafn đã có **application-layer security xuất sắc**:
...
## So sánh: Hrafn vs PicoClaw vs production grade

| Tính năng | PicoClaw | Hrafn hiện tại | Hrafn + lộ trình | Mục tiêu production |
|---------|----------|--------------|-------------------|-------------------|
...
| Tự kiểm tra bảo mật (`hrafn audit --check`) | 1 ngày | Thấp |
...
[security.audit]
enabled = true
log_path = "~/.config/hrafn/audit.log"
sign_events = true
max_size_mb = 100
...
hrafn security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Truy vấn audit log
hrafn audit --user @alice --since 24h
hrafn audit --risk high --violations-only
hrafn audit --verify-signatures
...
**Hrafn đã an toàn hơn PicoClaw** với:
...
**Khi triển khai lộ trình này**, Hrafn sẽ trở thành:
...
**Giá trị**: biến Hrafn từ "an toàn để kiểm thử" thành "an toàn cho production"
