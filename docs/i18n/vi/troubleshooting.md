...
- `cargo check` / `cargo build` dừng lâu ở `Checking hrafn`
...
### Không tìm thấy lệnh `hrafn` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `hrafn`
...
- Dừng các tiến trình `hrafn daemon` / `hrafn channel start` thừa
