...
- 如果不可避免需要类似身份的上下文，仅使用 Hrafn 范围的角色/标签（例如 `HrafnAgent`、`HrafnOperator`、`hrafn_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`HrafnAgent`、`HrafnOperator`、`HrafnMaintainer`、`hrafn_user`
    - 服务/运行时标签：`hrafn_bot`、`hrafn_service`、`hrafn_runtime`、`hrafn_node`
    - 环境标签：`hrafn_project`、`hrafn_workspace`、`hrafn_channel`
