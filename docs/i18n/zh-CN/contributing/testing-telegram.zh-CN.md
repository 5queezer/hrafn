...
```bash
# 检查配置
cat ~/.hrafn/config.toml | grep -A 5 telegram
...
```bash
# 启用调试日志
RUST_LOG=debug hrafn channel start
...
| 二进制大小 | ~3-4MB | `ls -lh target/release/hrafn` |
| 健康检查 | <5s | `time hrafn channel doctor` |
| 首次响应 | <3s | Telegram 中手动测试 |
| 消息拆分 | <50ms | 检查调试日志 |
| 内存使用 | <10MB | `ps aux \| grep hrafn` |
...
5. **监控日志** 查看任何问题：
   ...
   hrafn daemon
   # 或
   RUST_LOG=info hrafn channel start
   ```
...
- Issue：<https://github.com/5queezer/hrafn/issues>
