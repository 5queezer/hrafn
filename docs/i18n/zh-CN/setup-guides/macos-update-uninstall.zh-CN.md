...
本页面记录了 macOS（OS X）上 Hrafn 支持的更新和卸载流程。
...
- Homebrew：`/opt/homebrew/bin/hrafn`（Apple Silicon）或 `/usr/local/bin/hrafn`（Intel）
- Cargo/引导安装/手动安装：`~/.cargo/bin/hrafn`
...
```bash
command -v hrafn || echo \"hrafn 二进制文件未找到\"
pgrep -fl hrafn || echo \"没有运行中的 hrafn 进程\"
```
