...
本指南介绍 Hrafn 的原生 Nextcloud Talk 集成。
...
- 设置 `HRAFN_NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
hrafn daemon
# 或
hrafn gateway --host 127.0.0.1 --port 3000
```
...
- Hrafn 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- Hrafn 忽略非消息/系统事件。
