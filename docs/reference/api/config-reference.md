# Hrafn Config Reference (Operator-Oriented)
...
1. `HRAFN_WORKSPACE` override (if set)
2. persisted `~/.hrafn/active_workspace.toml` marker (if present)
3. default `~/.hrafn/config.toml`

Hrafn logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `hrafn config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `hrafn` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `hrafn doctor traces --limit 20`
  - `hrafn doctor traces --event tool_call_result --contains \"error\"`
  - `hrafn doctor traces --id <trace-id>`
...
1. `HRAFN_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `HRAFN_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- Each fallback provider resolves credentials independently using the standard resolution order: explicit config → provider-specific env var → `HRAFN_API_KEY` → `API_KEY`.
...
- `api_keys` supplies additional API keys that Hrafn rotates through on `429` (rate limit) responses. The primary `api_key` (set globally or per-channel) is tried first.
...
- When `enabled = true` and no OTP secret exists, Hrafn generates one and prints an enrollment URI once.
...
| `state_file` | `~/.hrafn/estop-state.json` | Persistent estop state path |
...
- Use CLI command `hrafn estop` to engage and `hrafn estop resume` to clear levels.
...
- Security-first default: Hrafn does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `HRAFN_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `HRAFN_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `HRAFN_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `HRAFN_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `hrafn skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- Hrafn requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
| `path_prefix` | _(none)_ | URL path prefix for reverse-proxy deployments (e.g. `"/hrafn"`) |

When deploying behind a reverse proxy that maps Hrafn to a sub-path,
set `path_prefix` to that sub-path (e.g. `"/hrafn"`). All gateway
...
3. Validate with `hrafn doctor` before restart/rollout.
...
- While `hrafn channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `HRAFN_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
| `bot_name` | Optional | Display name of the bot in Nextcloud Talk (e.g. `"hrafn"`). Used to filter out the bot's own messages and prevent feedback loops. |

Notes:

- Webhook endpoint is `POST /nextcloud-talk`.
- `HRAFN_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
