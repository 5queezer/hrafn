//! `hrafn update` — self-update pipeline with rollback.
...
pub async fn run(target_version: Option<&str>, include_pre: bool, force: bool) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("hrafn_new");
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    ...
    // Release assets are .tar.gz archives containing a single `hrafn` binary.
...
/// Extract the `hrafn` binary from a `.tar.gz` archive.
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        // The archive contains a single binary named "hrafn" (or "hrafn.exe" on Windows).
        let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");

        if file_name == "hrafn" || file_name == "hrafn.exe" {
            let mut buf = Vec::new();
            entry
                .read_to_end(&mut buf)
    ...
    bail!("archive does not contain a 'hrafn' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("hrafn") {
        bail!("downloaded binary does not appear to be hrafn");
    }
...
mod tests {
    ...
    fn extract_tar_gz_finds_binary() {
        ...
        // Build a tar.gz in memory containing a fake "hrafn" binary.
        ...
        let mut tar_buf = Vec::new();
        {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "hrafn", &fake_binary[..])
                .unwrap();
        ...
        let dest = tmp.path().join("hrafn_extracted");
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "hrafn".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
